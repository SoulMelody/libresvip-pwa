...
function Z() {
  ...
  var u = {
    ...
    relative: function(e, t) {
      ...
      for (var a = e.length, d = a - s, r = 1; r < t.length && t.charCodeAt(r) === 47; ++r)
        ;
      for (var l = t.length, g = l - r, S = d < g ? d : g, v = -1, _ = 0; _ <= S; ++_) {
        if (_ === S) {
          if (g > S) {
            ...
            if (_ === 0)
              return t.slice(r + _);
          } else d > S && (e.charCodeAt(s + _) === 47 ? v = _ : _ === 0 && (v = 0));
          break;
        }
        var P = e.charCodeAt(s + _), R = t.charCodeAt(r + _);
        if (P !== R)
          break;
        P === 47 && (v = _);
      }
      var b = "";
      for (_ = s + v + 1; _ <= a; ++_)
        (_ === a || e.charCodeAt(_) === 47) && (b.length === 0 ? b += ".." : b += "/..");
      return b.length > 0 ? b + t.slice(r + v) : (r += v, t.charCodeAt(r) === 47 && ++r, t.slice(r));
    },
    ...
    parse: function(e) {
      ...
      a ? (t.root = "/", d = 1) : d = 0;
      for (var r = -1, l = 0, g = -1, S = !0, v = e.length - 1, _ = 0; v >= d; --v) {
        if (s = e.charCodeAt(v), s === 47) {
          if (!S) {
            l = v + 1;
            break;
          }
        ...
        g === -1 && (S = !1, g = v + 1), s === 46 ? r === -1 ? r = v : _ !== 1 && (_ = 1) : r !== -1 && (_ = -1);
      }
      return r === -1 || g === -1 || // We saw a non-dot character immediately before the dot
      _ === 0 || // The (right-most) trimmed path component is exactly '..'
      _ === 1 && r === g - 1 && r === l + 1 ? g !== -1 && (l === 0 && a ? t.base = t.name = e.slice(1, g) : t.base = t.name = e.slice(l, g)) : (l === 0 && a ? (t.name = e.slice(1, r), t.base = e.slice(1, g)) : (t.name = e.slice(l, r), t.base = e.slice(l, g)), t.ext = e.slice(r, g)), l > 0 ? t.dir = e.slice(0, l - 1) : a && (t.dir = "/"), t;
    },
    sep: "/",
    delimiter: ":",
    win32: null,
  ...
  return u.posix = u, I = u, I;
}
var ee = Z(), M = /* @__PURE__ */ Y(ee);
const z = "/home/pyodide", H = (o) => `${z}/${o}`, C = (o, c) => o == null ? M.resolve(z, c) : M.resolve(H(o), c);
function J(o, c) {
  const f = M.normalize(c), i = M.dirname(f).split(/(?=\/)/);
  let e = "";
  for (const t of i) {
    e += t;
    const s = o.FS.analyzePath(e);
    if (s.exists && s.object) {
      if (!o.FS.isDir(s.object.mode))
...
function N(o, c, f, u) {
  J(o, c), o.FS.writeFile(c, f, u);
}
function te(o, c, f) {
  J(o, f), o.FS.rename(c, f);
}
const re = "[", oe = "(<=>!~", ne = ";", se = "@", ae = new RegExp(`[${re + oe + ne + se}]`);
function ie(o) {
...
function q(o) {
  return o.forEach((f) => {
    ...
    if (u.protocol === "emfs:" || u.protocol === "file:")
      throw new Error(`"emfs:" and "file:" protocols are not allowed for the requirement (${f})`);
  }), o.filter((f) => ie(f) === "streamlit" ? (console.warn(`Streamlit is specified in the requirements ("${f}"), but it will be ignored. A built-in version of Streamlit will be used.`), !1) : !0);
}
async function le(o) {
...
async function fe(o, c, f, u, i) {
  const { files: e, archives: t, requirements: s, prebuiltPackageNames: a, wheels: d, installs: r, pyodideUrl: l = o, streamlitConfig: g, idbfsMountpoints: S, nodefsMountpoints: v, moduleAutoLoad: _, env: P, languageServer: R } = f, b = q(s);
  if (O)
    ...
    const m = [];
    d && (m.push(d.streamlit), m.push(d.stliteLib)), R && m.push("jedi"), b.unshift(...m), console.debug("Loaded Pyodide");
  }
  const n = (
  ...
  })), v && Object.entries(v).forEach(([m, y]) => {
  ...
  await Promise.all(Object.keys(e).map(async (m) => {
    ...
    m = C(c, m);
    let k;
    "url" in y ? (console.debug(`Fetch a file from ${y.url}`), k = await fetch(y.url).then((A) => A.arrayBuffer()).then((A) => new Uint8Array(A))) : k = y.data, console.debug(`Write a file "${m}"`), N(n, m, k, e.opts), m.endsWith(".py") && h.push(m);
  ...
  if (i("Mocking some packages."), console.debug("Mock pyarrow"), de(n), console.debug("Mocked pyarrow"), i("Installing packages."), console.debug("Installing the prebuilt packages:", a), await n.loadPackage(a), console.debug("Installed the prebuilt packages"), console.debug("Installing the requirements:", b), await w.install.callKwargs(b, { keep_going: !0 }), console.debug("Installed the requirements"), r && (console.debug("Installing the additional requirements"), await Promise.all(r.map(({ requirements: m, options: y }) => {
    ...
    const m = h.map((y) => n.FS.readFile(y, { encoding: "utf8" }));
    W(n, u, m);
  }
...
__setup_script_finished_callback__`))(y), console.debug("Set up the IndexedDB filesystem synchronizer");
  ...
  if (R) {
    ...
    try {
      T = await n.pyimport("jedi"), console.debug("Loaded Jedi");
    } catch (m) {
...
async function $(o, c, f) {
  ...
  const t = o.pyimport("stlite_lib.server.Server")(u, c ? H(c) : void 0);
...
function B(o, c, f, u) {
  ...
  const a = async (d) => {
    ...
    if (r.type === "initData") {
      ...
      console.debug("Initial data", p), t = fe(o, u, p, e, i), t.then(({ pyodide: h }) => (i("Booting up the Streamlit server."), s = $(h, u, p.entrypoint), s)).then(() => {
        c({
    ...
    const l = await t, g = l.pyodide, S = l.micropip, v = l.jedi, { moduleAutoLoad: _ } = l.initData, P = await s, R = d.ports[0];
    function b(n) {
      R.postMessage(n);
    }
    try {
      switch (r.type) {
        case "reboot": {
          ...
          P.stop(), console.debug("Booting up the Streamlit server"), s = $(g, u, n), await s, console.debug("Booted up the Streamlit server"), b({
            type: "reply"
          });
          break;
        }
        ...
        case "http:request": {
          ...
          const { request: n } = r.data, p = (h, w, F) => {
            const L = new Map(w.toJs()), E = F.toJs();
            console.debug({ statusCode: h, headers: L, body: E }), b({
              type: "http:response",
              data: {
                response: {
                  statusCode: h,
        ...
        case "file:write": {
          const { path: n, data: p, opts: h } = r.data, w = C(u, n);
          _ && typeof p == "string" && w.endsWith(".py") && (console.debug(`Auto install the requirements in ${w}`), W(g, e, [p])), console.debug(`Write a file "${w}"`), N(g, w, p, h), b({
            type: "reply"
          });
          break;
        }
        case "file:rename": {
          const { oldPath: n, newPath: p } = r.data, h = C(u, n), w = C(u, p);
          console.debug(`Rename "${h}" to ${w}`), te(g, h, w), b({
            type: "reply"
          });
          break;
        }
        case "file:unlink": {
          ...
          console.debug(`Remove "${p}`), g.FS.unlink(p), b({
            type: "reply"
          });
          break;
        }
        ...
        case "install": {
          const { requirements: n, options: p } = r.data, h = q(n);
          console.debug("Install the requirements:", h), await S.install.callKwargs(h, p ?? {}).then(() => {
        ...
        case "setEnv": {
          const { env: n } = r.data;
          g.pyimport("os").environ.update(g.toPy(n)), console.debug("Successfully set the environment variables", n), b({
            type: "reply"
          });
          break;
        }
        case "code_completion": {
          ...
          const { code: n, line: p, column: h } = r.data, w = await ge(v, n, {
            line: p,
            column: h
          });
          b({
        ...
        case "run_python": {
          const { code: n } = r.data;
          console.debug("Run python code", n);
          const p = await g.runPythonAsync(n);
          let h;
          p instanceof g.ffi.PyProxy ? (console.debug("The result is a PyProxy object"), h = p.toJs(), p.destroy(), console.debug("Converted the result to a JS object", h)) : (h = p, console.debug("The result is a JS primitive", h)), b({
      ...
      p.name = n.name, p.stack = n.stack, b({
        type: "reply",
        error: p
      });
    }
...
const U = "https://cdn.jsdelivr.net/pyodide/v0.28.2/full/pyodide.mjs";
if ("postMessage" in self)
  self.onmessage = B(U, (o, c) => c ? self.postMessage(o, c) : self.postMessage(o));
else {
  const o = [];
  self.onconnect = (c) => {
    let f;
    ...
    const u = c.ports[0];
    u.onmessage = B(U, (i, e) => e ? u.postMessage(i, e) : u.postMessage(i), void 0, f), u.start();
  };
}
var D = /* @__PURE__ */ Object.freeze({
  __proto__: null
});
//# sourceMappingURL=worker-BA5oCmMm.js.map
