(function(){"use strict";var O="/libresvip-pwa/assets/libresvip-2.7.1-py3-none-any-CJTgqjlR.whl",N="/libresvip-pwa/assets/pycryptodomex-3.23.0-cp37-abi3-pyemscripten_2026_0_wasm32-DHXB43jU.whl",E="/libresvip-pwa/assets/wanakana_python-1.2.2-py3-none-any-mmo05MrR.whl";function y(t,n,e){const i=t&&t!=="Error"?`${t}: ${n}`:n;return e?`${i}
...
json.dumps(payload)
`);return JSON.parse(e)}async function I(t,n){const e=await f(),i=[],a=new Set;for(const o of n.tasks){l(t,{type:"task-started",taskId:o.id});const u=`/tmp/libresvip-pwa/${o.id}`,j=`${u}/${M(o.name)}`,S=D(A(o.name),n.outputFormat,a),$=`${u}/${S}`;try{e.FS.mkdirTree(u),e.FS.writeFile(j,new Uint8Array(o.data));const h=await e.runPythonAsync(`
import traceback
...
input_path = Path(${JSON.stringify(j)})
